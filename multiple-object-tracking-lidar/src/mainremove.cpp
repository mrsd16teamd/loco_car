...
cout<<"Sfdasffffffffff";
cout<<"sfsfsfsfsfsfsfsf";
...
//if (all_correspondences.size() < 20 )
//{
  ...
  void occupancyGridToGridMap(const nav_msgs::OccupancyGrid& input)
{
	...
	std::string layer = "1";
	//sensor_msgs::PointCloud2 pointcl (new sensor_msgs::PointCloud2);
	if( grid_map::GridMapRosConverter::fromOccupancyGrid(input ,layer , gridMap ))
		
		cout<<"doooneee";

	// grid_map::GridMapRosConverter::toPointCloud( gridMap, layer, &pointcl);

	//convpcl = nh.advertise<sensor_msgs::PointCloud2> ("pointcl", 1);
