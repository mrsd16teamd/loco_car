                                  ...
                                  std::vector<double> &x_des, geometry_msgs::Point &obstacle_pos)
{
  ...
  ROS_INFO("Start state: %f, %f, %f, %f, %f, %f",
          x_start.pose.pose.position.x, x_start.pose.pose.position.y, theta,
  ...
  double x0[9] = {x_start.pose.pose.position.x, x_start.pose.pose.position.y, theta,
                   x_start.twist.twist.linear.x, x_start.twist.twist.linear.y,
                   x_start.twist.twist.angular.z, x_traj_saved_[(step_on_last_traj_)*9 + 6],
                   u_init[0], u_init[1]};

  ROS_INFO("estimate of current steering: %f", x_traj_saved_[(step_on_last_traj_)*9 + 6]);
...
{
  ROS_INFO("Start PlanFromCurrentStateILQR.");
